		...
		<div class="layout">
			<table>
				...
				<c:forEach var="recipe" items="${recipeList}">
					<tr>
						<td>${recipe.id }</td>
						<td><a href="/user/addView?id=${recipe.id}">${recipe.title }</a></td>
						<td>${recipe.mainMenu }</td>
						<td>${recipe.difficult }</td>
						<td>${recipe.cookTime }</td>
						<td>
			<a href="/user/recipeList/${recipe.id}" data-user-id ="${recipe.id}" type="button" value="레시피삭제"
			onclick="alert('레시피를 삭제하시겠습니까?');">레시피삭제</a>
			</td>
					</tr>
				</c:forEach>
				</tbody>
